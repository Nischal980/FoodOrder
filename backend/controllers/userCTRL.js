...
const userCTRL = {
  register: async (req, res) => {
    try {
      const { fullName, userName, email, password, rePassword, role, shopName, address } =
        req.body;
...
if (!emailRegex.test(email)) {
  return res.status(400).json({ msg: "Invalid Email Format." });
      ...
      if(role=== "user"){
        newUser.shopName = userName;  
      }
      if (role === "Seller") {
        const { shopName } = req.body;
        if (!shopName) {
          return res.status(400).json({ msg: "Shop Name is required for Seller role." });
        }
        const shopNameUnique=await User.findOne({ shopName });
        if (!shopNameUnique) {
          return res.status(400).json({ msg: "Shop Name is alerady taken." });
        }
      ...
      const link = `http://localhost:3000/verify-user/${newUser._id}/${token}`;
