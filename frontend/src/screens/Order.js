...
function Order() {
  ...
  const order = async (data) => {
    try {
      setLoading(true);
      const orderAddress = data.district.toLowerCase();
      if (!orderAddress.includes("kathmandu")) {
        // Show an alert and return without proceeding
        toast.error("Order is valid within Kathmandu only.");
        setLoading(false);
        return;
      }
      await API.post(
                ...
                <form>
                  ...
                  {activeStep === steps.length - 1 ? (
                   ...
                   <Button
                    ...
                    <Khalti loading={loading} order={order}/>
                  </>) : (
                    <Button
                      className={classes.button}
                      variant="contained"
